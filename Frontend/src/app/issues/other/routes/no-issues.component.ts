...
export class NoIssuesComponent implements OnInit, OnDestroy {
  ...
  async button3Listener() {
    ...
    this.backendService.getIssueDiagnositic('continue').subscribe((res) => {
      ...
      this.sharedService.setOtherApiResponseData({
        iptvServiceDetail: res?.result?.responseData?.iptvServiceDetail,
        voiceServiceDetail: res?.result?.responseData?.voiceServiceDetail,
        hsiServiceDetail: res?.result?.responseData?.hsiServiceDetail,
      });
      this.sharedService.setLoader(false);
